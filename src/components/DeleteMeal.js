import React from 'react';
import Navigation from './navigation/Navigation';
import meal1 from '../img/meals/meal1.jpg';

function DeleteMeal() {
  return (
    <div className="flex">
      <Navigation />
      <div className="mx-auto my-8 p-6 bg-white rounded-md">
        <h1 className="text-2xl font-bold mb-10">My Meal(s):</h1>
        <div className="flex flex-col">
           <div className="bg-white rounded-lg shadow-md border-gray-300 border p-7 mr-4 mb-4 flex items-center justify-between">
              <img src={meal1} alt="meal-img" className="w-25 h-20 object-cover rounded mr-4" />
              <h2 className="text-lg font-semibold flex-grow text-center">Title</h2>
              <button type="button" className="bg-red-500 text-white py-2 px-4 rounded">Delete</button>
           </div>
        </div>
      </div>
    </div>
  );
}

export default DeleteMeal;
